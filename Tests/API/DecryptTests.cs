namespace Wallet.Tests.Api
{
    ...
    {
        private const string BaseUrl = "https://wallet.lyndychivs.com/";
        private const string GooglePayDecryptResource = "GooglePay/Decrypt";
        ...
        {
            var request = new RestRequest(GooglePayDecryptResource, Method.Post);
            request.AddJsonBody(GetValidGooglePayDecryptionRequest());

            var response = _restClient.Execute<GooglePayDecryptionResponse>(request);

            Assert.Multiple(() =>
            {
                Assert.That(response.Data, Is.TypeOf<GooglePayDecryptionResponse>());
        ...
        public void Invalid_Private_Key_Returns_Response_Of_Bad_Request()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(response.Content, Is.EqualTo("\"GooglePayDecryptionRequest.PrivateKey The input is not a valid Base-64 string as it contains a non-base 64 character, more than two padding characters, or an illegal character among the padding characters.\""));
            });
        }
        ...
        public void Invalid_Encrypted_Token_Returns_Response_Of_Bad_Request()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.BadRequest));
                Assert.That(response.Content, Is.Not.Null);
                Assert.That(response.Content, Is.EqualTo("\"GooglePayDecryptionRequest.EncryptedToken Cannot parse JSON\""));
            });
        }

        private static GooglePayDecryptionRequest GetValidGooglePayDecryptionRequest()
        {
        ...
        private static GooglePayDecryptionResponse GetValidGooglePayDecryptionResponse()
        {
